...
`Greeting`リソースは一度書いただけで、Web API、CLI、Homebrewパッケージの3つとして動きました。
ビジネスロジックの重複はなく、修正も一箇所で済みます。

境界を越えても同じリソースが使えるのは、HTTP・コマンドライン・配布パッケージのどれもが
リソースへのアクセス手段にすぎないからです。開発環境と本番環境、そしてユーザーの手元の
環境の違いも、リソースの実装には現れません。
...
ドメインロジックはインターフェイスと結合していないので、1つのリソースをテストすれば
全ての境界をカバーできます。Web APIとCLIのパラメーター構造は同じです。gRPCやGraphQLの
ような新しい境界が必要になっても、リソースはそのまま使えます。

### 現代的な配布システムとの統合

HomebrewやComposerで配布すれば、利用者は実行環境を意識せずにツールを使えます。
