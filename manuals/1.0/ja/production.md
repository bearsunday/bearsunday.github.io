...
ビルドで[コンパイル](#compilation-recommended)し、コンパイル済みの成果物を配布します。起動時にすることが残っていないので、コールドスタートは成果物を読むだけになり、スケールアウトで増えるインスタンスはすべて同じ成果物から起動します。[Dockerマルチステージビルド](#docker-multi-stage)と[Phar](phar.html)がこの形です。

* コンパイルは依存関係の問題を見つけるとexitコード1を返します。CIのゲートにします
...
コンパイルせずに配布し、デプロイ先でコンパイルします。ヘルスチェック（warmup）のタイミングで`composer compile`を実行するか、最初の起動に任せます。書き込めるツリーなら未コンパイルの成果物はその場でコンパイルされ、`Compiled DI scripts on demand`のnoticeが出ます。パスや環境変数など実環境の実値がそのまま焼き込まれます。リリースは別のディレクトリに展開し、ヘルスチェックが通ってからトラフィックを入れます。インスタンスごとにコンパイルが走るため、成果物の同一性は事前コンパイルに劣ります。
