...
優れたキャッシュシステムはユーザー体験の質を向上し、資源利用コストと環境負荷を下げます。BEAR.Sundayは既存の単純なTTLによるキャッシュに加え、以下のキャッシュ機能をサポートします。
...
キャッシュ対象メソッドを選択したい場合はクラスに属性を指定しないで、メソッドに指定します。その場合はキャッシュ変更メソッドに`#[RefreshCache]`という属性を与えます。
...
どちらかの方法でアトリビュートを与えると、概要で紹介した全ての機能が適用されます。
