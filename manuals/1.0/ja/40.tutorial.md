...
`public/index.php`を以下のように書き換えます。
...
`_self`リンクでリンクすると、`app://self/weekday`リソースが自身にコピーさレます。

appリソースの`weekday`が`$body['weekday']`に代入され、その他に引数のyear, month, dayをbodyに追加しています。
