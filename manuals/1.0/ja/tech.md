...
重い後続処理はレスポンスの後ろに回せます。リソースはリクエストを受け取ると`202 Accepted`を即座に返し、インデックスの更新や通知の送信は、レスポンスがクライアントに渡ったあとに実行されます。リソースが宣言するのは「何を」遅延するかだけで、「いつ・どこで」実行するかはリソースの外側で決まります。並列実行と同じく、リソースのコードは変わりません（[遅延リソース実行](defer.html)、Alpha）。
...
どの操作を自由に呼んでよく、どの操作に人の確認が要るかは、RESTのメソッドの意味論が決めます。safeでidempotentなGETは自由に、状態を変える操作は`confirm`で確認を挟み、ALPSプロファイルの遷移の種別で提示するツール自体を絞ることもできます。新しい仕組みを足したのではなく、URI、ユニフォームインターフェース、型、JSON Schema、ALPSが最初からツール定義の形をしていた、ということです。
