...
この「What（何を）」と「How（どう）」の完全分離により、リソースコードを一切変更することなく、`#[Embed]`で埋め込まれた複数のリソースを並列に取得できます。10年前に書かれたリソースクラスも、アプリケーション境界で非同期実行モードを選択することで並列実行の恩恵を受けられます。

サーバー環境の制約に応じて、ext-parallel（スレッドプール）、Swoole（コルーチン）、mysqli（DBクエリのみ並列化）の3段階のソリューションが用意されており、いずれを選択してもリソースコードは変更不要です。開発時は通常のPHPエントリポイントでデバッグし、本番ではasyncエントリポイントまたはSwooleサーバーで並列実行に移行できます。
