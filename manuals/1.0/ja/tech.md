...
コンパイルはビルドの仕事です。オブジェクトグラフはデプロイの前に解決され、DIの設定エラーはビルドを止めて本番には届きません。起動は成果物を読むだけで、ツリーには何も書きません。だからツリーは読み取り専用のまま出荷でき、サーバーレスやイミュータブルコンテナのように書き込める場所が限られた環境でも、[書き込み先を宣言する](production.html#writable-paths)だけで動きます。コールドスタートにかかるのは成果物を読む時間だけで（リソース5個のアプリケーションで、再コンパイルが0.38秒、成果物からの読み込みが0.018秒）、スケールアウトで増えたインスタンスは全て同じ成果物から起動し、同じ答えを返します。

成果物は1ファイルの[Phar](phar.html)にできます。コード、`vendor/`、コンパイル済みDIスクリプトが1つのアーカイブに収まり、デプロイは1ファイルのコピー、ロールバックは1つ前のファイルです。何を入れるかはフレームワークが決め、コンパイルされていないビルドや自身に書き込むビルドはアーカイブになる前に止まります。インポートした複数のアプリケーションも同じアーカイブに入ります。静的ビルドしたPHPを隣に置けばホストにPHPをインストールする必要はなく、コマンドラインのアプリケーションならPHPとアーカイブを1つの実行ファイルにできます。同じアーカイブは[ブラウザの中でも動きます](wasm.html)。サーバーにPHPランタイムもアプリケーションサーバーも要りません。
...
サーバー環境の制約に応じて、ext-parallel（スレッドプール）とSwoole（コルーチン）の2つのランタイムが用意されており、どちらを選択してもアプリケーションコードは変更不要です。開発時は通常のPHPとしてデバッグし、本番では設定の切り替えだけで並列実行に移行できます。SQLも同じです。SQLごとにリソースを分けて上位のリソースから`#[Embed]`で束ねると、複数のSQLがランタイムに応じて並列に発行されます。利用側はリソースを組み合わせるだけです。

### 遅延リソース実行

重い後続処理はレスポンスの後ろに回せます。リソースは`202 Accepted`を返し、インデックスの更新や通知の送信は、レスポンスがクライアントに渡ったあとに実行されます。レスポンスが先に返るかはSAPIで決まり、PHP-FPMとLiteSpeedでは接続が解放されて即座に返り、Apache mod_phpではベストエフォートです。リソースが宣言するのは「何を」遅延するかだけで、「いつ・どこで」実行するかはリソースの外側で決まります。並列実行と同じく、リソースのコードは変わりません（[遅延リソース実行](defer.html)、Alpha）。
...
AIエージェントはBEAR.Sundayの規約に沿ってコードを生成し、レビューします。[BEAR.Skills](https://github.com/bearsunday/BEAR.Skills)はリソース生成、ハイパーメディア化、品質レビューといった作業をClaude Codeのスキルとして提供し、[BEAR.Kata](https://github.com/bearsunday/BEAR.Kata)は「これを実装したい時、どのファイルを見るか」を引くための実装パターン集です。詳細は[AIアシスタント](ai-assistant.html)を参照してください。
...
リソースはAIエージェントの道具にもなります。[BEAR.ToolUse](ai-assistant.html#beartooluse)はResourceObjectからTool定義を生成し、パラメーターの説明と制約はJSON Schema、PHPDoc、ALPSプロファイルから組み立てます。AIのために別の関数群を作るのではなく、アプリケーションの実際の機能がそのまま道具になります。

どの操作を自由に呼んでよく、どの操作に人の確認が要るかを決める根拠は、RESTのメソッドの意味論にあります。safeでidempotentなGETは自由に呼ばせ、状態を変える操作には`#[Tool(confirm: true)]`を付けて、束縛した確認ハンドラーが実行の前に人に問います。確認は認証や認可の代わりではなく、その上に置く人の判断の一段です。新しい仕組みを足したのではなく、URI、ユニフォームインターフェース、型、JSON Schema、ALPSが最初からツール定義の形をしていた、ということです。
...
* **使いやすさ**：
  優れた接続性により、他の言語やシステムとの円滑な連携が実現します。また、リソースをCLIツールとして提供することで、エンドユーザーは複雑な環境設定なしにアプリケーションの機能を利用できます。PHPを同梱した1つの実行ファイルとして配布することもできます。
