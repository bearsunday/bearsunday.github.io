...
サーバー環境の制約に応じて、ext-parallel（スレッドプール）、Swoole（コルーチン）、mysqli（DBクエリのみ並列化）の3段階のソリューションが用意されており、いずれを選択してもリソースコードは変更不要です。開発時は通常のPHPエントリポイントでデバッグし、本番では非同期エントリポイントまたはSwooleサーバーで並列実行に移行できます。
