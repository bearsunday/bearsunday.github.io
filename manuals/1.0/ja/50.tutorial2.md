...
Ray.MediaQueryを使えば、ボイラープレートとなりやすい実装クラスをコーディングする事がありません。インターフェイスから自動生成されたSQL実行オブジェクトがインジェクトされます。[^5]
...
インジェクトされた`$uuid`を文字列にキャストする事でUUIDの文字列表現が得られます。また`#Link[]`は他のリソース（アプリケーション状態）へのリンクを表します。
