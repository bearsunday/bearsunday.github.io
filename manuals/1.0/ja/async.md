...
BEAR.Asyncは`#[Embed]`リソースの透過的な並列実行を可能にします。リソースのコードはそのままに、アプリケーションの境界で実行モードを選ぶだけで、埋め込みリソースが自動的に並列に取得されます。async/await や Promise、yield などの記述は不要で、10年前に書かれたリソースクラスもそのまま並列実行の恩恵を受けられます。
