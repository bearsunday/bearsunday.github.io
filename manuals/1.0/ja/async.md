...
BEAR.Asyncは`#[Embed]`リソースの透過的な並列実行を可能にします。アプリケーションコードを変更することなく、埋め込みリソースを並列に取得します。10年前に書かれたリソースクラスも、実行モードを切り替えるだけで並列実行の恩恵を受けられます。
...
この`query://self/user_profile`は「ユーザーのプロファイル情報が欲しい」という意図だけを示しています。この「What（何を）」と「How（どう）」の分離により、同じコードが同期実行でも並列実行でも動作します。開発時は通常のPHPとしてXdebugでデバッグし、本番ではエントリポイントやモジュールを切り替えるだけで並列実行を有効化できます。
...
BEAR.Asyncはアプリケーションの境界（application boundary）で実行モードを選びます。リソースコード（`#[Embed]`グラフ）は変更しません。

| モード | エントリポイント | ランタイム要件 |
|-----|----------|------|
| ext-parallel | `bin/async.php` を追加 | ZTS PHP + ext-parallel |
| ext-swoole | `bin/swoole.php` + `AsyncSwooleModule` をインストール | ext-swoole |

ext-parallelはワーカーごとに独立したランタイム（別スレッド・別メモリ）を使うため、別のエントリポイントとして選択します。ext-swooleは単一プロセス内のコルーチンとして動作するため、AppModuleに通常のモジュールとしてインストールします。この非対称はランタイムモデルの違いから来ています。

### ext-parallel（PHP-FPM / Apache）

`bin/app.php`と並べて`bin/async.php`を追加します。ライブラリのbootstrapを呼び出し、通常のAppModuleにparallelランタイムをオーバーレイします:
...
parallelランタイムを`AppModule`に直接インストールしないでください。bootstrapが唯一のインストール経路であり、同じ`AppModule`が`bin/app.php`（同期）と`bin/async.php`（並列）の両方で変更なしに動作します。

ワーカープール数を上書きする場合は、6番目の引数で指定します（デフォルトはCPUコア数）:
...
ワーカーランタイムは独立したZendメモリを持つ別スレッドです。並列実行される`#[Embed]`は以下を満たす必要があります:

- **純粋／冪等** — 同じ入力から同じ出力が得られること。ワーカーはリクエストスコープの状態を共有しません（共有セッションも共有ロガーコンテキストもありません）。
- **各ワーカーが独立したDIコンテナを持つ** — `AppModule`のシングルトンはスレッド間で同一インスタンスにはなりません。並列化される`#[Embed]`の中で「同一インスタンスである」ことに依存しないでください。
- **ペイロードのコピー可能性** — スレッド境界を跨ぐ引数（現状は`query`配列）と戻り値（`$ro->body`またはレンダリング済み文字列）は、スカラー／null／それらのネスト配列でなければなりません。オブジェクト、クロージャ、リソースは`NonCopyablePayloadException`で即座に失敗します。
- **リクエストローカルな状態を変更するインターセプター** はワーカー境界を跨ぐと誤動作します。横断的関心事は冪等にするか、並列化される`#[Embed]`グラフの外でスコープしてください。

### ext-swoole（Swoole HTTPサーバー）

Swoole HTTPサーバー環境で高並行性を要求するアプリケーションに使います。`AsyncSwooleModule`を`AppModule`にインストールし、起動には`bin/swoole.php`を使います。
...
Swooleではコルーチンがメモリを共有するため、`PdoPoolEnvModule`による接続プールが必要です。クロススレッドのコピー可能性制約はSwooleには存在しません。
...
開発時は`bin/app.php`（同期）でXdebugを使ったデバッグを行い、本番では`bin/async.php`（並列）に切り替える、というように同期モードでのデバッグと本番での並列実行を使い分けられます。
