...
BEAR.Sundayのリソースには、Webアプリケーション以外からも同じリソースとしてアクセスできます。

- **HTTPクライアント**:
  HTTPで全てのリソースにアクセスできます。MVCのモデルやコントローラーと違い、BEAR.Sundayのリソースはクライアントから直接アクセス可能な設計です。

- **CLIツール**:
  リソースはコマンドラインツールとしても使えます。Homebrewで配布でき、PHPやBEAR.Sundayを知らなくても、通常のUNIXコマンドとして扱えます。アプリケーション本体のバージョン管理から独立しているため、CLIツールとしての安定性を保てます。

- **composerパッケージ**:
  composerでvendor下にインストールしたアプリケーションのリソースを直接呼び出せます。マイクロサービスにしなくても、複数のアプリケーションを協調させて動かせます。

- **多言語フレームワーク**:
  BEAR.Thriftを使えば、PHP以外の言語や異なるバージョンのPHPとも連携できます。

## Webキャッシュ

サーバーサイドのTTLキャッシュではなく、クライアント・CDN・サーバーサイドの三層にキャッシュを分散させます。Webがもともと持っていたキャッシュの仕組みをそのまま使います。
...
アプリケーションの起動は、DIコンテナを実行時に操作しません。依存の解決はコンパイル時に終わっており、起動時に生成するのはルートオブジェクト1つだけです。このオブジェクトはリクエストをまたいで再利用されます。
