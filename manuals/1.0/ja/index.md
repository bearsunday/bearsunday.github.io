...
リソースはWebアプリケーションの中だけのものではありません。次のクライアントから同じリソースを利用できます。
...
- **多言語フレームワーク**:
  BEAR.Thriftを使うと、PHP以外の言語や異なるバージョンのPHPからリソースを利用できます。

## Webキャッシュ

リソース指向アーキテクチャとモダンなCDNを組み合わせると、サーバーサイドのTTLキャッシュにとどまらない、Web本来の分散キャッシュが使えます。Webの基本原則に沿ってCDNを中心に据えることで、パフォーマンスと可用性の両方が上がります。
...
ブートストラップを徹底して切り詰めているため、ユーザー体験とシステムリソースの両方に効きます。
パフォーマンスはBEAR.Sundayの最重要課題の一つで、設計と開発の判断はここを基準に決めてきました。
