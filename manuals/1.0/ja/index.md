...
BEAR.Sundayのリソースは、Webブラウザ以外のクライアントからも同じ形でアクセスできます。

- **HTTPクライアント**:
  HTTPで全てのリソースにアクセスできます。MVCのモデルやコントローラーと違い、BEAR.Sundayのリソースはクライアントから直接アクセス可能な設計です。

- **CLIツール**:
  リソースはコマンドラインツールとしても使えます。Homebrewで配布でき、PHPやBEAR.Sundayを知らなくても、ふつうのUNIXコマンドとして扱えます。アプリケーション本体のバージョン管理から独立しているため、CLIツールとしての安定性を保てます。

- **composerパッケージ**:
  composerでvendor下にインストールしたアプリケーションのリソースを直接呼び出せます。マイクロサービスを立てなくても、複数のアプリケーションを協調して動かせます。
