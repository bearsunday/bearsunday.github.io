...
BEAR.Sundayの目標は、標準に準拠し高品質でAPI中心のRESTfulアプリケーションのフレームワークを提供することです。

BEAR.SundayとはPHPのWebアプリケーションフレームワークです。
その目標は、標準に準拠し、高品質でAPI中心のRESTfulアプリケーションを開発するための堅牢なフレームワークを提供することです。
BEAR.Sundayは、アプリケーションの長期的な維持管理のしやすさ、優れた拡張性、メンテナンス性、パフォーマンスを実現するように設計されています

## フレームワーク

**BEAR.Sunday**は3つの強力なオブジェクトフレームワークで構成されています。
...
- **クリーンな設計**: コンポーネントとの密結合を避け、DIとAOPによる疎結合を実現します。これにより、ライブラリや他のコンポーネントとの統合が容易になります。
...
- **堅牢性**:
  イベントドリブンコンテンツの採用により、CDNを中心としたキャッシュシステムは、PHP やDBがダウンした場合でもコンテンツを提供し続けます。
...
BEAR.Sundayは、Webの本質であるリソースを中心に設計された、PHPのWebアプリケーションフレームワークです。その真の価値は、オブジェクト指向原則とREST原則に基づいた優れた制約をアプリケーション全体の制約として提供することにあります。
