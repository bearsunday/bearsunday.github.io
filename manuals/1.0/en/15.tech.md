...
The Acyclic Dependencies Principle states that dependencies should be unidirectional and non-circular. Following this principle, each package within the BEAR.Sunday framework maintains a unidirectional dependency relationship without relying on a central framework package. Specifically, the application depends on the BEAR.Package framework package, and BEAR.Package depends on the BEAR.Sunday interface core package. Furthermore, BEAR.Sunday depends on BEAR.Resource ROA package, BEAR.Resource depends on Ray.Di, and Ray.Di depends on Ray.Aop. In this way, the dependent side does not need to know the details of the side it depends on. The dependencies are unidirectional and do not form cycles.

As backward compatibility is maintained, each package can be updated independently. Additionally, there is no framework version number that locks the entire system, as seen in other frameworks. There is also no mechanism for object proxies that hold cross-cutting dependencies between objects.

This Acyclic Dependencies Principle is aligns with the principles of DI (Dependency Injection), and the root object generated during the bootstrapping process of BEAR.Sunday is also constructed following this principle.
