...
Dependency Injection (DI) is a pattern that automatically resolves object creation and dependencies by the framework, reducing code coupling and improving testability and maintainability. BEAR.Sunday's DI uses Ray.Di, an independent package that incorporates the design philosophy of Google's Guice DI framework and covers almost all its features.
...
* Composition is recommended instead of using inheritance classes. By design, the only abstract class that requires inheritance at runtime is `BEAR\Resource\ResourceObject`. However, the principle is not to call parent class methods from inherited classes, and necessary things are resolved in principle by composition. In other words, the minimal methods held by ResourceObject are not called by the inherited child class, but are used by other classes on principle. In fact, there is not a single situation where a method provided by the framework is used.
